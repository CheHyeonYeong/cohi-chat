...
- Message owner branches should import `ChatModule` and inject the shared `ChatRoomActivityNotifier` token. This branch binds that token to `ChatPollNotifier`.
...
7. While the poll request is waiting, the sender-side message flow should call `ChatRoomActivityNotifier.notifyRoomActivity(roomId)` after commit so the wait returns immediately.
