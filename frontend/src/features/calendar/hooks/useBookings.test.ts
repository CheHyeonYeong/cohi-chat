...
describe('useMyBookings', () => {
    ...
    it('uses a username-scoped cache key so another user cannot reuse previous booking data', async () => {
        ...
        queryClient.setQueryData(calendarKeys.myBookings(1, 10, 'alice'), aliceData);
        ...
        expect(queryClient.getQueryData(calendarKeys.myBookings(1, 10, 'alice'))).toEqual(aliceData);
        expect(queryClient.getQueryData(calendarKeys.myBookings(1, 10, 'bob'))).toEqual(bobData);
    });
});
