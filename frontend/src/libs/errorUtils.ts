    ...
    if (error instanceof Error) {
        ...
        if (typeof status === 'number' && status >= 500) {
            if (import.meta.env.DEV) {
                console.error('[getErrorMessage] 원본 에러:', error);
            }
            return HTTP_STATUS_MESSAGES[status] ?? fallback;
        }

        // 브라우저 네트워크 에러 패턴 → 사용자 친화적 메시지로 대체
        const networkErrorPattern = /^(Failed to fetch|NetworkError|Load failed)/;
        if (networkErrorPattern.test(error.message)) {
            if (import.meta.env.DEV) {
                console.error('[getErrorMessage] 원본 에러:', error);
            }
 ...
 */
export const isHttpError = (error: unknown, status: number): boolean => error instanceof Error && error.cause === status;
